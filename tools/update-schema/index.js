...
try {
  ...
  console.warn(
    "Couldn't find config.json, so falling back to environment variables."
  );
  config = process.env;
}
...
async function start() {
  ...
  const { body: currentSchema } = await request(
    `https://raw.githubusercontent.com/telegramdesktop/tdesktop/master/Telegram/SourceFiles/mtproto/scheme/api.tl?${Date.now()}`
  );
  ...
  const { body: latestSchema } = await request(
    `https://raw.githubusercontent.com/telegramdesktop/tdesktop/dev/Telegram/SourceFiles/mtproto/scheme/api.tl?${Date.now()}`
  );
  ...
  if (Number.isNaN(currentLayerNumber) || newLayerNumber > currentLayerNumber) {
    console.log(
      "Current layer is NaN or new > current, starting PR process..."
    );
    ...
    // await exec(
    //   `git clone --depth=1 git@github.com:tjhorner/schema.tl.git ${config.REPO_WORKING_DIRECTORY}`
    // );
    ...
    await fs.writeFile(
      path.join(config.REPO_WORKING_DIRECTORY, "resources", "schema.json"),
      JSON.stringify(newSchema, null, 2)
    );
    await fs.writeFile(
      path.join(config.REPO_WORKING_DIRECTORY, "resources", "schema.tl"),
      latestSchema
    );

    console.log(
      "Ensuring all documentation files for methods/constructors exist..."
    );
    ...
    newSchema.methods.forEach(async (method) => {
      ...
      try {
        ...
        console.log(
          `Writing empty doc file for new method ${method.method}...`
        );
    ...
    newSchema.constructors.forEach(async (constructor) => {
      ...
      try {
        ...
        console.log(
          `Writing empty doc file for new constructor ${constructor.predicate}...`
        );
    ...
    var schemaJs = (
      await fs.readFile(
        path.join(config.REPO_WORKING_DIRECTORY, "js", "schema.js")
      )
    ).toString();
    schemaJs = schemaJs.replace(
      /var LAYER_NUMBER = [0-9]+/,
      `var LAYER_NUMBER = ${newLayerNumber}`
    );
    schemaJs = schemaJs.replace(
      /var SCHEMA_GLOBAL = {[\s\S]+}/,
      `var SCHEMA_GLOBAL = ${JSON.stringify(newSchema, null, 2)}`
    );
    ...
    console.log(
      "For my final trick, updating the layer number on the index page..."
    );

    var indexHtml = (
      await fs.readFile(path.join(config.REPO_WORKING_DIRECTORY, "index.html"))
    ).toString();
    indexHtml = indexHtml.replace(
      /TL-Schema Explorer \(Layer [0-9]+\)/,
      `TL-Schema Explorer (Layer ${newLayerNumber})`
    );

    await fs.writeFile(
      path.join(config.REPO_WORKING_DIRECTORY, "index.html"),
      indexHtml
    );
    ...
    // await exec(
    //   `git add -A && git -c "user.name=${config.GIT_NAME}" -c "user.email=${config.GIT_EMAIL}" commit -m "[chore] Update to layer ${newLayerNumber}"`,
    //   { cwd: config.REPO_WORKING_DIRECTORY, shell: "bash" }
    // );
    ...
    console.log(
      "New layer is the same as current layer. Nothing to change! Bye"
    );
  }
}
